...
pub fn tools() -> Vec<ToolDef> {
    vec![
        ...
        tool!(
            "refill_zones",
            "Recompute and write every copper pour fill. Runs headlessly via kicad-cli — \
             KiCAD does not need to be open; if it is, it is reloaded afterwards. Call this \
             after adding or editing zones, since a pour's copper is only generated by a fill.",
            json!({
                "type": "object",
                "properties": {
                    "board": { "type": "string", "description": "Path to .kicad_pcb file" },
